    ...
    <ul class="nav nav-pills flex-column" style="margin-top:50px;" id="menu">
      <li class="sidebar-item">
        ...
        <ul id="categories" class="sidebar-dropdown list-unstyled collapse"  data-bs-parent="#sidebar">
          <li class="sidebar-item">
            <router-link 
              :class="{ 'nav-link': true, 'sidebar-link': true, 'active': isCurrentRoute('/categories/dresses/') }" 
              to="/dresses" aria-label="Dresses page">
              <strong v-if="isCurrentRoute('/categories/dresses/')">/DRESSES/</strong>
              <span v-else>/DRESSES/</span>
            </router-link>
          </li>
          <li>
            <router-link 
              :class="{ 'nav-link': true, 'sidebar-link': true, 'active': isCurrentRoute('/categories/shirts/') }" 
              to="/shirts" aria-label="Shirts page">
              <strong v-if="isCurrentRoute('/categories/shirts/')">/SHIRTS/</strong>
              <span v-else>/SHIRTS/</span>
            </router-link>
          </li>
          <li>
            <router-link 
              :class="{ 'nav-link': true, 'sidebar-link': true, 'active': isCurrentRoute('/categories/skirts/') }" 
              to="/skirts" aria-label="Skirts page">
              <strong v-if="isCurrentRoute('/categories/skirts/')">/SKIRTS/</strong>
